...
Public Class REPORTE_VENTAS_POR_VENDEDOR_VS_FECHA
    ...
    Private Sub REPORTE_VENTAS_POR_VENDEDOR_VS_FECHA_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
                    ...
                    Else
                        If V_RESUMIDO = 1 Then
                            Try
                                ...
                                Dim info As New INF_VENTAPORVEND_PORMES_RESUMIDO
                    ...
                    Else
                        If V_RESUMIDO = 1 Then
                            Try
                                conectar()
                                Dim dt As New DataTable
                                Dim da As New SqlDataAdapter("REPORTE_VENTAS_PORVENDEDOR_TODOSMES", SQLconexion)
                                da.SelectCommand.CommandType = CommandType.StoredProcedure
                                da.SelectCommand.Parameters.AddWithValue("@ANHO", year)
                                da.SelectCommand.Parameters.AddWithValue("@USUARIO", v_usuario)
                                da.Fill(dt)

                                Dim ds As New Data.DataSet
                                ds.Tables.Add(dt)

                                Dim info As New INF_VENTAPORVEND_TODOMESES_RESUMIDO

                                info.SetDataSource(ds)
                                info.SetParameterValue("@ANHO", year)
                                info.SetParameterValue("@USUARIO", v_usuario)
                                SetDBLogonForReport(iconexion, info)
                                Me.crw.ReportSource = info
                    ...
                    If PorMes = 1 Then
                        If V_DETALLADO = 1 Then
                            Try
                                conectar()
                                Dim dt As New DataTable
                                Dim da As New SqlDataAdapter("REPORTE_VENTAS_PORVENDEDOR_PORMES", SQLconexion)
                                da.SelectCommand.CommandType = CommandType.StoredProcedure
                                da.SelectCommand.Parameters.AddWithValue("@FECHA_INI", FECHA_INI)
                                da.SelectCommand.Parameters.AddWithValue("@FECHA_FIN", FECHA_FIN)
                                da.SelectCommand.Parameters.AddWithValue("@USUARIO", v_usuario)
                                ...
                                Dim info As New INF_VENTAPORVEND_PORMES
