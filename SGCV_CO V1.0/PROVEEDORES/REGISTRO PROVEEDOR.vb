Public Class REGISTRO_PROVEEDOR
    ...
    '** VERIFICAR EXISTENCIA DE DATOS DE PROVEEDOR*****
    Public Function YaExisteProvee(ByVal a As String) As Integer
        Try
            conectar()
            Dim sel As String
            sel = "SELECT COUNT(*) FROM TP_PROVEEDOR WHERE RUC = '" & a & "'"
            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
            SQLconexion.Open()
            Dim t As Integer = CInt(cmm.ExecuteScalar())
            cmm.Dispose()
    ...
    Private Sub txtRUC_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtRUC.Leave
        If YaExisteProvee(Trim(Me.txtRUC.Text)) = 1 Then
            MessageBox.Show("Ya hay Proveedor con este numero de RUC", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Me.txtRUC.Focus()
        Else
            Dim texto As String
            'If Not IsNumeric(txtRUC.Text) Then
            '    MsgBox("Se debe ingresar solo RUC con digito -  ")
            'Else
