...
Public Class GESTIONAR_COBRO
    ...
    Dim DaBanco, DaDetRec, DaDetCuenta_Cliente, DaInteres, DaUsuario, DaDatosCliente As New SqlClient.SqlDataAdapter
    Dim DsBanco, DsDetRec, DsDetCuenta_Cliente, DsInteres, DsUsuario As New Data.DataSet
    ...
    Dim VALOR_CODCHEQUE, B_PAGOREALIZADO As Integer
    Dim PARA_PAGOCUOTAS, PARA_PAGOINTERES, AUX_PAGO, AUX_PAGO1, INTERES_MORATORIO_Rec As Integer
    Dim DetallesCheque, CEDULA_CLIENTE, NOMBRE_CLIENTE, CLIENTE_, cobrador, DatosClie_paraCaja As String
    '
    Dim saldoXfactura As Integer
    ...
    Function DATOS_CLIENTE1(ByVal a As Integer)
        Try
            conectar()
            SQLconexion.Open()
            Dim sel As String
            ...
            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
            'crear adapter
            DaDatosCliente = New SqlClient.SqlDataAdapter(cmm)
            'cargamos en el datatable
            Dim DATOS_CLIENTE_ As New DataTable
            DaDatosCliente.Fill(DATOS_CLIENTE_)
            SQLconexion.Close()

            Return DATOS_CLIENTE_
    ...
    Function paraInteresenCuenta(ByVal a As String, ByVal b As Integer, ByVal c As Integer) As Integer
        Try
            conectar()
            Dim sel As String
            sel = "SELECT COALESCE(INTERES_MORATORIO,0) FROM VF_DETALLE_CUENTACLIENTE " & _
            "WHERE DOCUMENTACION_FACTURACION = '" & a & "' AND CUOTA = " & b & " AND COD_CLIENTE = " & c & ""
            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
            SQLconexion.Open()
            Dim t As Integer = CInt(cmm.ExecuteScalar())
            cmm.Dispose()
    ...
    Function BUSQUEDA_IDENTIFICADOR(ByVal A As String, ByVal B As String) As String
        Try
            conectar()
            Dim sel As String
            sel = "SELECT IDENTIFICADOR FROM VF_CABECERA_FACTURA WHERE COD_CLIENTE = " & A & " AND NUMERO_FACTURA = '" & B & "'"
            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
            SQLconexion.Open()
            Dim t As String = CStr(cmm.ExecuteScalar())
            cmm.Dispose()
    ...
    Sub VER_DATOS_CLIENTE_CONTADO(ByVal codigo_cliente As Integer)

        Dim cod_cliente_ As Integer = codigo_cliente
    ...
    Sub VER_DATOS_CLIENTE_CREDITO(ByVal codigo_cliente As Integer)
    ...
    Private Sub dgDetalleCuentCli_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgDetalleCuentCli.Click
        ...
        Else
            ...
            IDENTIFICADOR2 = BUSQUEDA_IDENTIFICADOR(CODIGO_CLIENTE, VALOR_DOCUMENTO)
    ...
    Function Contador_Recibo() As Integer
        Try
            conectar()
            Dim sel As String
            sel = "SELECT MAX(COD_CABECERA_RECIBO) FROM CP_CABECERA_RECIBO"
            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
            SQLconexion.Open()
            Dim t As Integer = CInt(cmm.ExecuteScalar())
            cmm.Dispose()
    ...
    Function Contador_Detalle_() As Integer
        Try
            conectar()
            Dim sel As String
            sel = "SELECT COUNT(COD_DETALLE_RECIBO) FROM CP_DETALLE_RECIBO"
            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
            SQLconexion.Open()
            Dim t As Integer = CInt(cmm.ExecuteScalar())
            cmm.Dispose()
    ...
    Private Sub btnRealizarPago_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRealizarPago.Click
        ...
        If identificador_dobleclick = 1 Then
                ...
                Else
                    ...
                    Else
                        If CInt(Me.txtMontoPagoCuotas.Text) > CInt(Me.txtImportedeCuotas.Text) Then
                            MessageBox.Show("IMPORTE DE PAGO DEBE SER MENOR O IGUAL AL IMPORTE DE CUOTA", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            Me.txtMontoPagoCuotas.Focus()
                        Else
                            If cobrador = "" Then
                                MessageBox.Show("DEBE SELECCIONAR UN COBRADOR", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                                Me.txtClienteCI.Focus()
                            Else
            ...
            If VERIFICAR_PARA_PAGO = 1 Then
                ...
                Else
                    If TIPO_DOCUMENTO = "CREDITO" Then
                        ...
                        Dim InteresMora_Historial, InteresMora_Cuenta, importe_cu, saldo_interes, auxiliar_monto, para_Cuota, INTERES_MORATORIO_Cuen, cantidad_cadena As Integer
                        ...
                        For i = 0 To Me.BindingContext(Me.DsDetRec, "CP_DETALLE_RECIBO").Count - 1
                            ...
                            If (contador_cabRecibo) = CInt(Me.DsDetRec.Tables("CP_DETALLE_RECIBO").Rows(Me.BindingContext(Me.DsDetRec, "CP_DETALLE_RECIBO").Position).Item("COD_CABECERA_RECIBO").ToString) Then
                                ...
                                cantidad_cadena = auxiliar_cadena.Length
                                If cantidad_cadena <= 4 Then
                                    cuota_ = auxiliar_cadena.Substring(0, 1)
                                Else
                                    cuota_ = auxiliar_cadena.Substring(0, 2)
                                End If
                                ...
                                INTERES_MORATORIO_Rec = paraInteresMora(contador_cabRecibo, "INTERES MORATORIO CUOTA", auxiliar_cadena)
                                'INTERES_MORATORIO_Cuen = paraInteresenCuenta(valor_doc, cuota_, CODIGO_CLIENTE)
                                ...
                                If valor_doc <> "INTERES MORATORIO CUOTA" Then
                                    ...
                                    If PARA_PAGOCUOTAS >= importe_cu Then
                                        ...
                                        'CALCULAR MONTOS PARA PAGOS DE INTERES
                                        If CInt(Me.txtMontoPagoInteres.Text) > INTERES_MORATORIO_Rec Then
                                            saldo_interes = CInt(Me.txtMontoPagoInteres.Text) - INTERES_MORATORIO_Rec
                                            Me.txtMontoPagoInteres.Text = CStr(saldo_interes)
                                            InteresMora_Cuenta = 0
                                            InteresMora_Historial = INTERES_MORATORIO_Rec
                                        Else
                                            saldo_interes = INTERES_MORATORIO_Rec - CInt(Me.txtMontoPagoInteres.Text)
                                            InteresMora_Cuenta = saldo_interes
                                            InteresMora_Historial = CInt(Me.txtMontoPagoInteres.Text)
                                        End If
                                        ...
                                        Try
                                            ...
                                            "FECHA_PAGO = '" & Today & "',INTERES_MORATORIO = " & InteresMora_Cuenta & " " & _
                                    ...
                                    Else
                                        ...
                                        Try
                                            ...
                                            "FECHA_PAGO = '" & Today & "',MONTO_CUOTA = " & AUX_PAGO1 & ",INTERES_MORATORIO = " & InteresMora_Cuenta & " " & _
                                            "WHERE DOCUMENTO_FACTURACION = '" & valor_doc & "' AND " & _
                                            "CUOTA = " & para_Cuota & " AND COD_CLIENTE = " & CODIGO_CLIENTE & ""
                                            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
                                            SQLconexion.Open()
                        ...
                        '** VERIFICADOR DE BANDERA DE PAGO REALIZADO  ****
                        B_PAGOREALIZADO = 1
            ...
            Me.txtClienteCI.Show()
            Me.lstnombre.Show()
            Me.dgDetalleCuentCli.Hide()
    ...
    Function ContadorDetalleEnCaja_(ByVal a As Integer) As Integer
        Try
            conectar()
            Dim sel As String
            sel = "SELECT COALESCE(MAX(COD_DET_CAJA),0) FROM CP_DETALLE_CAJA WHERE COD_CAB_CAJA = " & a & ""
            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
            SQLconexion.Open()
            Dim t As Integer = CInt(cmm.ExecuteScalar())
            cmm.Dispose()
    ...
    'VERIFICA SI YA HUBO CIERRE PARA CERRAR!!!
    Function Operacion_Pendiente(ByVal a As Integer) As Integer
        Try
            Dim sel As String

            sel = "SELECT coalesce(max(IMPORTE_TOTAL),1) FROM CP_CABECERA_RECIBO WHERE COD_CABECERA_RECIBO = " & a & ""
            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
    ...
    Private Sub btnSalir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSalir.Click
        ...
        If Operacion_Pendiente(contador_cabRecibo) = 0 Then
            MessageBox.Show("TIENE OPERACION PENDIENTE CIERRE DE PAGO, FAVOR CERRAR EL CICLO DE PAGO", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Me.btnRealizarPago.Focus()
        Else
            If Operacion_Pendiente(contador_cabRecibo) = 1 Then
                Me.Close()
            Else
                Me.Close()
            End If
    ...
    Private Sub btnCancelarOperacion_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCancelarOperacion.Click
        ...
        If MessageBox.Show("Desea Cancelar la Operacion?", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.YesNo, MessageBoxIcon.Question
                           ) = vbYes Then
            Try
                conectar()
                Dim sel As String
        ...
        Else
            ...
            Else
                ...
                Try
                    ...
                    'Dim info As New RECIBOTK_DE_DINERO_
                    info.SetDataSource(ds)
                    info.SetParameterValue("@NUMERO_RECIBO", NUM_RECIBO)
                    SetDBLogonForReport(iconexion, info)
                    Me.CrystalReportViewer1.ReportSource = info
                ...
                Try
                    ...
                    Dim info As New RECIBOTK_DE_DINERO_1
                    info.SetDataSource(ds)
                    info.SetParameterValue("@NUMERO_RECIBO", NUM_RECIBO)
                    SetDBLogonForReport(iconexion, info)
                    info.PrintOptions.PrinterName = impresosaPredt
            ...
            If IDENTIFICADOR2 = "T" Then
                If EsConGarante(CODIGO_CLIENTE) = 0 Then
                    Try
                        ...
                        Dim info As New REPORTE_PAGARE_A_LA_ORDEN_SIN_FACTURA
                ...
                Else
                    Try
                        ...
                        Dim info As New REPORTE_PAGARE_A_LA_ORDEN_CON_GARANTE_SIN_FACTURA
    ...
    Private Sub dgDetalleCuentCli_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgDetalleCuentCli.DoubleClick
        ...
        Else
            ...
            CODIGO_FACTURA = BUSQUEDA_CODFACTURA(CODIGO_CLIENTE)
            '***********************************************************************
            Dim DATOS_CLIENTE As New DataTable
            DATOS_CLIENTE = DATOS_CLIENTE1(CODIGO_CLIENTE_)
            CEDULA_CLIENTE = DATOS_CLIENTE.Rows(0)("CI").ToString
            NOMBRE_CLIENTE = DATOS_CLIENTE.Rows(0)("NOM_APE").ToString
            DatosClie_paraCaja = CEDULA_CLIENTE & "  " & NOMBRE_CLIENTE & "  " & "CUOTA N° " & CUOTA
            '***********************************************************************
            ...
            If TIPO_DOCUMENTO = "CONTADO" Then
                ...
                Else
                    ...
                    End If
                End If
                'VISUALIZAR MONTOS DE LA FACTURA.
                Me.txtImportedeCuotas.Text = CStr(Puntos(MONTO_CUOTA))
                Me.txtImportedeInteres.Text = CStr(0)
            End If
    ...
    Private Sub txtMontoPagoCuotas_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtMontoPagoCuotas.KeyPress
    ...
    Private Sub txtMontoPagoInteres_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtMontoPagoInteres.KeyPress
        If Char.IsDigit(e.KeyChar) Then
            e.Handled = False
        ElseIf Char.IsControl(e.KeyChar) Then
            e.Handled = False
    ...
    Private Sub txtDescuentoCuota_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtDescuentoCuota.KeyPress
        If Char.IsDigit(e.KeyChar) Then
            e.Handled = False
        ElseIf Char.IsControl(e.KeyChar) Then
            e.Handled = False
    ...
    Private Sub txtDescuentoInteres_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtDescuentoInteres.KeyPress
        If Char.IsDigit(e.KeyChar) Then
            e.Handled = False
        ElseIf Char.IsControl(e.KeyChar) Then
            e.Handled = False
    ...
    Private Sub txtClienteCI_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtClienteCI.TextChanged
        If Me.rbCedula.Checked = True Then
            Try
                conectar()
                SQLconexion.Open()
                Dim sel As String
                sel = "SELECT * FROM TP_CLIENTE WHERE CI LIKE '%" & Trim(Me.txtClienteCI.Text) & "%'"
                cmm = New SqlClient.SqlCommand(sel, SQLconexion)
                'crear adapter
        ...
        Else
            If Me.rbRuc.Checked = True Then
                Try
                    ...
                    SQLconexion.Open()
                    Dim sel As String
                    sel = "SELECT * FROM TP_CLIENTE WHERE RUC LIKE '%" & Trim(Me.txtClienteCI.Text) & "%'"
                    cmm = New SqlClient.SqlCommand(sel, SQLconexion)
                    'crear adapter
                    DaCliente = New SqlClient.SqlDataAdapter(cmm)
                    'crear dataset
            ...
            Else
                If Me.rbNombreApellido.Checked = True Then
                    Try
                        ...
                        SQLconexion.Open()
                        Dim sel As String
                        sel = "SELECT * FROM TP_CLIENTE WHERE NOM_APE LIKE '%" & Trim(Me.txtClienteCI.Text) & "%'"
                        cmm = New SqlClient.SqlCommand(sel, SQLconexion)
                        'crear adapter
                        DaCliente = New SqlClient.SqlDataAdapter(cmm)
                        'crear dataset
    ...
    Private Sub lstnombre_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lstnombre.Click
        CLIENTE_ = Trim(Me.lstnombre.Text)
        Dim cedula_cliente As String = cedula_cliente_(CLIENTE_)
        'SE EMPIEZA CON LA BUSQUEDA DE LOS DATOS
        DsDetRec.Clear()
        ...
        pendiente = "PENDIENTE"
        CODIGO_CLIENTE = CodigoCli(Trim(cedula_cliente))
        ...
        '*********************************************************************************************************
        If Me.rbContado.Checked = True Then
            VER_DATOS_CLIENTE_CONTADO(CODIGO_CLIENTE)
        Else
            If Me.rbCredito.Checked = True Then
                VER_DATOS_CLIENTE_CREDITO(CODIGO_CLIENTE)
            End If
        End If

        Me.txtClienteCI.Hide()
        Me.lstnombre.Hide()
        Me.dgDetalleCuentCli.Show()
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Me.txtClienteCI.Clear()
        Me.lstnombre.ValueMember = Nothing
        Me.txtClienteCI.Show()
        Me.lstnombre.Show()
        Me.dgDetalleCuentCli.Hide()
    End Sub
    ...
    Private Sub cbCobrador_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbCobrador.SelectedIndexChanged
        cobrador = Trim(Me.DsUsuario.Tables("CONFIG_USUARIO").Rows(Me.cbCobrador.SelectedIndex).Item(2).ToString)
    End Sub
