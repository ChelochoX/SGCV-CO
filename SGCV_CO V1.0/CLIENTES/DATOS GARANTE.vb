Public Class DATOS_GARANTE
    ...
    Private Sub DATOS_GARANTE_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Me.txtDatosTitular.Text = Datos_TitularGarante
    ...
    Public Function YaExisteCLiente(ByVal a As String) As Integer
        Try
            conectar()
            Dim sel As String
            sel = "SELECT COUNT(*) FROM TP_CLIENTE_GARANTE WHERE CI = '" & a & "'"
            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
            SQLconexion.Open()
            Dim t As Integer = CInt(cmm.ExecuteScalar())
            cmm.Dispose()
    ...
    Private Sub btnQuitarGA_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnQuitarGA.Click
        Try
            conectar()
            Dim sel As String
            sel = "DELETE FROM TP_CLIENTE_GARANTE WHERE COD_CLIENTE_GARANTE = " & codigo & ""
            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
            SQLconexion.Open()
            Dim t As Integer = CInt(cmm.ExecuteScalar())
            cmm.Dispose()
