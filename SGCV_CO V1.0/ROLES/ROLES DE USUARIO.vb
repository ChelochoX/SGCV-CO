Public Class ROLES_DE_USUARIO
    ...
    Private Sub ROLES_DE_USUARIO_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Me.cbUsuario.DisplayMember = "USUARIO"
    ...
    Function Estado_Usuario(ByVal a As String, ByVal b As String) As String
        Try
            conectar()
            Dim sel As String
            sel = "SELECT ESTADO FROM CONFIG_USUARIO WHERE USUARIO = '" & a & "' AND SUCURSAL = '" & b & "' "
            cmm = New SqlClient.SqlCommand(sel, SQLconexion)
            SQLconexion.Open()
            Dim t As String = CStr(cmm.ExecuteScalar())
            cmm.Dispose()
    ...
    Private Sub btnCambiarEstado_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCambiarEstado.Click
        ...
        If (MessageBox.Show("PRESIONA (SI) PARA HABILITAR O PRECIONA (NO) PARA DESABILTIAR", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.YesNo, MessageBoxIcon.Information)) = DialogResult.Yes Then
            If Me.txtContrasena1.Text = "" Then
                MessageBox.Show("INGRESAR CONTRASEÑA PARA USUARIO", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.txtContrasena1.Focus()
            Else
                If Me.txtContrasena2.Text = "" Then
                    MessageBox.Show("VOLVER A INGRESAR CONTRASEÑA", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Me.txtContrasena2.Focus()
                Else
                    If Trim(Me.txtContrasena1.Text) <> Trim(Me.txtContrasena2.Text) Then
                        MessageBox.Show("CONTRASEÑAS NO COINCIDEN,VOLVER A INGRESAR", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        Me.txtContrasena1.Focus()
                    Else
                        If categoria = "" Then
                            MessageBox.Show("DEFINIR CATEGORIA AL USUARIO", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            Me.GroupBox1.Focus()
                        Else
                            Dim usuario_ As String = "HABILITADO"
                            Try
                                Dim sel As String
            ...
            End If
        Else
            Dim usuario_ As String = "BLOQUEADO"
            Try
                Dim sel As String
        ...
        If Estado_Usuario(usuario, empresa) = "BLOQUEADO" Then
        ...
        Else
            If Estado_Usuario(usuario, empresa) = "HABILITADO" Then
            ...
            Else
                ...
                Else
                    Try
                        ...
                        MessageBox.Show("CONTRASEÑA MODIFICADA", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        Me.txtContrasena1.Clear()
                        Me.txtContrasena2.Clear()
                        Me.txtContrasena1.Focus()
    ...
    Private Sub cbUsuario_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbUsuario.SelectedIndexChanged
        usuario = CStr(Me.DsUsuario.Tables("CONFIG_USUARIO").Rows(Me.cbUsuario.SelectedIndex).Item(6).ToString)
    ...
    Private Sub btnCategoria_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCategoria.Click
        ...
        If Me.rbAdmin.Checked = True Then
            Try
                Dim sel As String
                sel = "UPDATE CONFIG_USUARIO SET CATEGORIA ='" & categoria & "'" & _
                "WHERE USUARIO ='" & Trim(usuario) & "'"
                cmm = New SqlClient.SqlCommand(sel, SQLconexion)
                SQLconexion.Open()
                Dim t As Integer = CInt(cmm.ExecuteScalar())
                ...
                MessageBox.Show("CATEGORIA MODIFICADA", "SGCV_CO VERSION EXTENDIDA", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Catch ex As Exception
                MsgBox(ex.Message())
                SQLconexion.Close()
            End Try
